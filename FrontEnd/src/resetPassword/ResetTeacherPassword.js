...
//Reset Detail
  async function handleSubmit(e) {
    try {
      e.preventDefault()
        const response = await axios.post("http://localhost:5555/teacher/reset", {
          email: email
        })
  ...
  async function handleVerification(e) {
    try{
      e.preventDefault()
        ...
        const response = await axios.post('http://localhost:5555/teacher/verify', {verificationCode: verificationCode}, {headers})
      ...
      if(error.response.data.status === 'Invalid verification code!'){
          setDoubleCheck(true)
      }    
    }
  }
  ...
  async function handleUpdate(e, email) {
    ...
    try {
      ...
      if(password !== repeatPassword) {
        ...
        const response = await axios.put(`http://localhost:5555/teacher/update/${email}`, {password})
