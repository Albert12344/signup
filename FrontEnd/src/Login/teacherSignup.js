...
//Create timer for resend Verification code
 ...
 //Post Signup details 
  async function handleSubmit(e) {
    try {
    e.preventDefault()
      const response = await axios.post('http://localhost:5555/teacher/signup', {
...
//Resend Verification Code  
  async function handleResend(e) {
    e.preventDefault()
    try {
    const response = await axios.post('http://localhost:5555/teacher/resend', {
      email  
    })    
          ...
          <form className='form'>
            <h1 className='signup'>Գրանցուել</h1>
              ...
              <label className='label'>Գաղտնաբառ</label><br />
                ...
                <br />
                  <Link className='label' to='/teacher/login'>Մուտք</Link>
          </form>
        </div>
      }
      {/* Verification Bar */}
